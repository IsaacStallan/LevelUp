    ...
    <header className="relative border-b border-white/10 sticky top-0 backdrop-blur-md bg-white/[0.03] z-10">
      <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link to="/" className="text-lg font-bold text-white tracking-tight logo-glow">
            ⚡ LevelUp
          </Link>
      ...
      {menuOpen && (
        <div className="sm:hidden absolute left-0 right-0 top-full backdrop-blur-md bg-gray-950/95 border-b border-white/10 anim-menu-slide shadow-xl z-20">
          ...
          <nav className="px-2 py-2">
            {[
              { to: '/',        icon: '🏠', label: 'Dashboard',     cls: 'text-gray-300' },
              { to: '/habits',  icon: '📋', label: 'Habits',        cls: 'text-gray-300' },
              { to: '/upgrade', icon: '⚡', label: 'Upgrade — $7/mo', cls: 'text-purple-400' },
            ].map(item => (
              <Link
                key={item.to}
                to={item.to}
